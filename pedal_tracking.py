...
def track_tape(video_path, output_path="tracked_video.mp4", csv_output_path="tracked_data.csv"):
    ...
    Only the top 40% of the image and the rightmost 40% to 20% of the screen is considered for tracking.
    ...
    try:
        # Open the video file
        cap = cv2.VideoCapture('croppedleft.mp4')        # INSERT NAME OF VIDEO FILE HERE
        if not cap.isOpened():
        ...
        # Define dark orange/yellow color range for tape detection (in HSV color space)
        lower_orange = np.array([10, 100, 20])  # Adjusted lower bound for darker color
        upper_orange = np.array([20, 255, 100])  # Adjusted upper bound for darker color
        ...
        while True:
            ...
            roi_height = int(height * 0.40)  # Top 40%
            roi_y_start = 0  # Start y-coordinate of the ROI
            roi_width = int(width * 0.20)    # Right 20% of the image
            roi_x_start = int(width * 0.80)                # Start x-coordinate of the ROI

            # Create a mask for the top 40% of the image and right 20%
            mask = np.zeros_like(hsv[:, :, 0])
            mask[roi_y_start:roi_y_start + roi_height, roi_x_start:width] = 255

            # Apply the dark orange/yellow color mask within the ROI mask
